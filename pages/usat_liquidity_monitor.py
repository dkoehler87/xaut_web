...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 27 04:39:19 2026
...
def compute_liquidity_metrics(orderbook: dict, depth_bps_levels: list[float], depth_unit: str, depth_anchor: str):
    """
    depth_anchor:
      - "best_bid/ask": current behavior
          bid cutoff = best_bid*(1 - X)
          ask cutoff = best_ask*(1 + X)
      - "mid": symmetric around mid
          bid cutoff = mid*(1 - X)
          ask cutoff = mid*(1 + X)
    """
    ...
    for bps in depth_bps_levels:
        ...
        if depth_anchor == "mid":
            bid_cutoff = mid * (1.0 - band)
            ask_cutoff = mid * (1.0 + band)
        else:
            # default: best_bid/ask
...
def parse_bps_list(text: str, default=None):
    ...
    for part in (text or "").split(","):
        ...
        try:
            ...
            pass
    out = sorted(set(out))
    return out[:20] if out else default


HISTORY_KEY = "usat_liq_history"
...
def prune_history(minutes: int):
    ...
    st.session_state[HISTORY_KEY] = [
        r for r in st.session_state[HISTORY_KEY]
...
def liquidity_page():
    st.title("USAT Liquidity Monitor")
    ...
    depth_unit = st.sidebar.selectbox(
        ...
        ["quote", "base"],
        index=0,
        help="quote = sum(price*amount) in USD/USDT terms, base = sum(amount) in USAT",
    )
    ...
    if st_autorefresh is not None:
        st_autorefresh(interval=refresh_seconds * 1000, key="usat_liq_refresh")
    else:
        st.info("Optional: `pip install streamlit-autorefresh` to auto-poll without manual refresh.")


    # ----------------------------
    # Depth anchor toggle (must be defined BEFORE we fetch snapshots)
    # ----------------------------
        ...
        for pair in VENUES[venue]["pairs"]:
            ...
            try:
                ...
                metrics = compute_liquidity_metrics(
                                                    ...
                                                    depth_bps_levels,
                                                    depth_unit,
                                                    depth_anchor="mid" if depth_anchor == "Mid" else "best_bid/ask",
                                                )
    ...
    if rows:
        st.session_state[HISTORY_KEY].extend(rows)
        prune_history(window_minutes)

    live_df = pd.DataFrame(rows)
    hist_df = pd.DataFrame(st.session_state[HISTORY_KEY])
    ...
    if not live_df.empty:
        ...
        st.caption(f"Depth anchor: {depth_anchor}")
