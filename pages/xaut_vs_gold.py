...
def require_xaut_page_password() -> None:
    ...
    if hmac.compare_digest(entered_password, configured_password):
        ...
        st.session_state.pop("xaut_vs_gold_lockout_until", None)
        st.rerun()

    failed_attempts = int(st.session_state.get("xaut_vs_gold_failed_attempts", 0)) + 1
    st.session_state["xaut_vs_gold_failed_attempts"] = failed_attempts
...
def validate_oanda_stream_url(stream_url: str) -> str:
    ...
    if (
        parsed.scheme != "https"
        or parsed.hostname not in OANDA_ALLOWED_STREAM_HOSTS
        ...
        raise ValueError("OANDA_STREAM_URL must be an official OANDA HTTPS streaming endpoint.")
